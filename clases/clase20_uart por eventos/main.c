    ...
    while(1){
        ...
        if (xQueueReceive(uart_queue,(void *)&event,portMAX_DELAY))
        {
            ...
            case  UART_DATA:
                ...
                uart_flush(UART_NUM);//limpiar el buffer de entrada para que no hay overflow
                ESP_LOGI(tag, "Data received: %s",data);

                for (size_t i = 0; i < event.size; i++) //event.size-2 ,ya que la cadena de cadena de caracteres tiene \n y \r
                {
                    char value=data[i];
